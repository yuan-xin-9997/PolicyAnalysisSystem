...
正文以纯文本安全渲染，不执行来源 HTML、JavaScript 或内嵌资源；正文按换行/空行切分为段落分段显示，并对每个段落首行缩进两个汉字宽度。
...
入库前由采集器对正文进行清洗：剥离网页页面装饰信息（开头的面包屑、栏目、时间与「来源：」页眉行，以及结尾的「阅读下一篇」推荐区块）并归一化段落空白。清洗在计算内容哈希与写入正文前完成，去重与存储均基于清洗后的纯政策正文；清洗不改变采集判定阈值与分类规则（判定仍基于原始正文）。已入库政策的历史正文不回填，仅在下次重新抓取时随 upsert 更新。
