...
已入库政策的历史正文可使用 `scripts/reclean_policy_content.py` 重洗：脚本对每条 `content_text` 应用新版清洗规则、重算 `content_hash`，仅更新确有变化的行（幂等，可重复执行），`policies_fts_au` 触发器同步全文索引；支持 `--dry-run` 预览。
