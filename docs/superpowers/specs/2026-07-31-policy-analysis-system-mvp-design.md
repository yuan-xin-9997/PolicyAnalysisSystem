...
7. 推送管理占位页面；政策分析已实现词频分析模块（见 2.4）。
...
- 实际邮件推送；
- 表述差异分析、主题分类、政策相似度、政策趋势与大模型分析（词频分析已交付，见 2.4）；
- 新华网之外的政府网站采集适配器；
...
在首期 MVP 之后，政策分析页面从占位页升级为词频分析模块：基于选中政策正文，后台异步执行 jieba 中文分词、停用词过滤、词频统计、TF-IDF（以选中政策集合为语料，平滑公式 `log((1+N)/(1+df)) + 1`）与关键词共现计算（TOP-N 两两共同出现篇数），结果按任务持久化。前端提供词频排行、词云（ECharts wordCloud）与关键词关系图（ECharts Graph）三视图，并展示历史任务。任务在独立线程池执行（`analysis.max_workers`，默认 1），与采集任务解耦；单次任务政策数上限 `analysis.max_policies_per_task`（默认 100）。NLP 为纯函数 `engine.py`，便于后续扩展主题分类、相似度与趋势分析。需求与决策见 OpenSpec 变更 `2026-08-09-policy-word-frequency-analysis`。

### 2.5 政策比对分析模块（增量交付）

政策数据库沿用跨页多选状态，选择至少两篇不同政策后可创建 `policy_comparison` 异步任务。分析引擎对正文执行与词频模块一致的分词和停用词过滤，按词频提取各篇核心关键词，以余弦相似度衡量两篇政策的文本特征接近程度，并输出共同关键词和双方独有关键词。选择 N 篇时生成 N×(N−1)/2 个两两比对结果。报告以结构化 JSON 持久化到 `analysis_comparison_reports`，前端展示政策概览、共同关注点与两两差异，并支持历史回看。
...
正文以纯文本安全渲染，不执行来源 HTML、JavaScript 或内嵌资源；正文按换行/空行切分为段落分段显示，并对每个段落首行缩进两个汉字宽度。正文段落结构由采集端从抓取 HTML 的 `<p>` 元素还原（见 7.3），存量无换行正文安全兜底为单段渲染。
...
入库前由采集器对正文恢复段落结构并清洗：WebFetch 的 `generic.article` 适配器会把正文压成单行空格拼接、丢失 `<p>` 段落，因此采集判定通过后额外调用 `fetch_text` 取回原始 HTML，优先解析 `<div id="detail">` 内的 `<p>` 块还原段落（缺失时回退扁平正文）；随后剥离网页页面装饰信息--包括开头的面包屑（含「新华网 > > 正文」双 `>` 变体）、栏目、带空格时间戳（如「2022 12/ 14」）与「来源：」页眉行，以及结尾的编辑署名（「策划：」「监制：」「新华社音视频部制作」「新华通讯社出品」等）、「【纠错】」「【责任编辑:xxx】」标记、「阅读下一篇：N …」推荐区块和尾部纯数字跟踪 ID--并归一化段落空白。清洗在计算内容哈希与写入正文前完成，去重与存储均基于段落化清洗后的纯政策正文；清洗不改变采集判定阈值与分类规则（判定仍基于原始正文）。段落抓取失败或解析为空时自动回退扁平正文并记录告警日志，绝不阻断采集。

已入库政策的历史正文可使用 `scripts/reclean_policy_content.py` 重洗，脚本对每条 `content_text` 重算 `content_hash`，仅更新确有变化的行（幂等，可重复执行），`policies_fts_au` 触发器同步全文索引。两种模式：默认扁平清洗仅对存量 `content_text` 应用清洗规则（剥离页脚/工具栏/编辑署名，但适配器预置的标题使页眉不在行首、行首锚定正则无法剥离，且无法分段）；`--refetch` 模式按 `canonical_url` 重新抓取原始 HTML，从 `<p>` 元素还原段落结构（排除位于 `<p>` 之外的标题与页眉），彻底去页眉并恢复多段正文，抓取失败时安全回退扁平清洗。
