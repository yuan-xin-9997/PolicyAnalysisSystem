...
- 政策详情以纯文本显示正文，按换行/空行切分为段落分段渲染并对每段首行缩进两个汉字宽度，不解析或执行抓取内容中的 HTML。
...
采集器在入库前会对正文进行清洗，剥离网页页面装饰信息（开头的面包屑、栏目、时间与「来源：」页眉行，以及结尾的「阅读下一篇」推荐区块）并归一化段落空白；清洗在计算内容哈希与写入正文前完成，使去重与存储均基于纯政策正文，且不改变采集判定阈值。已入库政策的历史正文不回填，仅在下次重新抓取时随 upsert 更新为清洗后内容。
